...
export async function upsertProspectBot({ email, pack, botKey, name, metierLabel, lang, phone = "", address = "", website = "", trialDays = 7 }) {
  ...
  await pool().query(
    `INSERT INTO bots (
     ...
     ON CONFLICT (public_id) DO UPDATE SET
       ...
       trial_end=CASE
         WHEN bots.paid=1 THEN bots.trial_end
         ELSE LEAST(COALESCE(bots.trial_end, EXCLUDED.trial_end), EXCLUDED.trial_end)
       END,
       blocked=CASE WHEN bots.paid=1 THEN 0 ELSE bots.blocked END`,
...
export async function markBotPaid(publicId, { customerId, subscriptionId, status }) {
  await pool().query(
    `UPDATE bots SET paid=1, blocked=0, stripe_customer_id=$2, stripe_subscription_id=$3, stripe_status=$4
     WHERE public_id=$1`,
    [publicId, customerId || null, subscriptionId || null, status || "active"]
  );
}

export async function blockUnpaidBot(publicId, status = "expired") {
  if (!publicId) return;
  await pool().query(
    `UPDATE bots SET blocked=1, stripe_status=$2 WHERE public_id=$1 AND paid=0`,
    [publicId, status]
  );
}

export async function markBotUnpaid(publicId, status = "canceled") {
  if (!publicId) return;
  await pool().query(
    `UPDATE bots SET paid=0, blocked=1, stripe_status=$2 WHERE public_id=$1`,
    [publicId, status]
  );
}

...
// Requête Pexels courte et pertinente : les premiers mots-clés du imagePrompt
// du métier (déjà écrit en registre "photo stock") + la ville.
function buildPexelsQuery(metier, metierId, ville) {
  const base = (metier?.imagePrompt || metierId || "business").split(",")[0].trim();
  return ville ? `${base} ${ville}` : base;
}

export async function generateHeroImageUrl(metierId, nom_enseigne, ville, lang = "fr") {
  ...
  if (!apiKey) {
    ...
    const pexels = await fetchPexelsPhoto(buildPexelsQuery(metier, metierId, ville));
  ...
  const basePrompt =
    metier?.imagePrompt ||
    (lang === "en"
      ? `professional background photo for the ${metierId} business in ${ville}`
